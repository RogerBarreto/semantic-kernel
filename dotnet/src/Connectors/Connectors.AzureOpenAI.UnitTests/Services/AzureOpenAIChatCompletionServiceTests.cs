...
public sealed class AzureOpenAIChatCompletionServiceTests : IDisposable
{
    ...
    public async Task GetChatMessageContentsHandlesSettingsCorrectlyAsync()
    {
        ...
        Assert.Equal(0.6, content.GetProperty("temperature").GetDouble(), 0.001);
        Assert.Equal(0.5, content.GetProperty("top_p").GetDouble(), 0.001);
        Assert.Equal(1.6, content.GetProperty("frequency_penalty").GetDouble(), 0.001);
        Assert.Equal(1.2, content.GetProperty("presence_penalty").GetDouble(), 0.001);
