...
public sealed class BedrockChatClientAutoFunctionChoiceBehaviorTests : BaseIntegrationTest
{
    private readonly Kernel _kernel;
    private readonly FakeFunctionFilter _autoFunctionInvocationFilter;
    private readonly IChatClient _chatClient;

    public BedrockChatClientAutoFunctionChoiceBehaviorTests()
    {
        this._autoFunctionInvocationFilter = new FakeFunctionFilter();

        this._kernel = this.InitializeKernel("anthropic.claude-3-5-sonnet-20240620-v1:0");
    ...
    public async Task SpecifiedInCodeInstructsConnectorToInvokeNonKernelFunctionManuallyAsync()
    {
        ...
        var settings = new PromptExecutionSettings() { FunctionChoiceBehavior = FunctionChoiceBehavior.Auto([plugin.ElementAt(0)], autoInvoke: false) };
        var chatOptions = settings.ToChatOptions(this._kernel);
    ...
    public async Task SpecifiedInCodeInstructsAIModelToCallFunctionInParallelOrSequentiallyAsync(bool callInParallel)
    {
        ...
        var settings = new PromptExecutionSettings() { FunctionChoiceBehavior = FunctionChoiceBehavior.Auto(options: new() { AllowParallelCalls = callInParallel }) };
        var chatOptions = settings.ToChatOptions(this._kernel);
    ...
    private Kernel InitializeKernel(string modelId)
        => this.CreateKernelBuilder()
            .AddBedrockChatClient(modelId)
            .Build();
    ...
    private readonly IConfigurationRoot _configuration = new ConfigurationBuilder()
        ...
        .AddUserSecrets<BedrockChatClientAutoFunctionChoiceBehaviorTests>()
        .Build();
...
#pragma warning restore CA1812 // Avoid uninstantiated internal classes
    {
        ...
        public string GetCurrentDate(string what) => DateTime.UtcNow.ToString("d", CultureInfo.InvariantCulture);
    }
